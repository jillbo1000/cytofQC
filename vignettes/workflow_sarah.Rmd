...
```{r}
cytofHist(x = tech[, "Bead1"], group = beads$init, "Bead channel")
```
...
```{r}
cytofHist(x = tech[, "Bead1"], group = beads$init, type = "density", 
          title = "Bead channel")
```
...
index[ind] <- TRUE
cytofHist(x = tech[, "Bead1"], group = index, type = "density", 
          title = "Bead channel")
```
...
```{r}
tmp <- data.frame(bead = tech[, "Bead1"], group = rf.labs$label)
tmp <- tmp[tmp$bead > 0, ]
cytofHist(tmp$bead, group = tmp$group, type = "count", title = "Bead channel")
```
...
```{r}
cytofHist(tech[, "Bead1"], rf.labs$label, title = "Final labels on Bead measures")
tmp <- data.frame(bead = tech[, "Bead1"], group = rf.labs$label)
tmp <- tmp[tmp$bead > 0, ]
cytofHist(tmp$bead, tmp$group, title = "Final labels on Bead measures - zeros removed")
...
```{r}
g.dat <- CATALYST::prepData(fname)
gfp <- g.dat@assays@data$exprs[rownames(g.dat@assays@data$exprs) == "GFP", ]
vericells <- g.dat@assays@data$exprs[rownames(g.dat@assays@data$exprs) == "VeriCells", ]
...
umapData$gfp <- log(gfp)
umapData$vericells <- vericells
...
```{r}
umaps[[6]] <- ggplot(umapData, aes(x = x, y = y, color = gfp)) +
    geom_point(size = 0.5, show.legend = FALSE) +
    scale_colour_gradient(low = "#56B1F7", high = "#132B43", space = "Lab",
                          na.value = "white", guide = "colourbar", 
                          aesthetics = "colour") +
    # scale_color_continuous("", type = "gradient", values = "reds") +
    labs(title = "GFP", x = "", y = "") +
    theme_bw()
```

```{r}
umaps[[7]] <- ggplot(umapData, aes(x = x, y = y, color = vericells)) +
    geom_point(size = 0.5, show.legend = FALSE) +
    scale_colour_gradient(low = "#f6eff7", high = "#016450", space = "Lab",
                          na.value = "white", guide = "colourbar", 
                          aesthetics = "colour") +
    # scale_color_continuous("", type = "gradient", values = "reds") +
    labs(title = "Vericells", x = "", y = "") +
    theme_bw()
```
...
grid.arrange(umapG + guides(col = "none"), umaps[[1]], umaps[[2]], 
             umaps[[3]], umaps[[4]], umaps[[5]], umaps[[6]], 
             umaps[[7]], nrow = 4, ncol = 2)
```
