...
Histograms can be created using `cytofHist`. The function creates a histogram colored by groups. The following uses `cytofHist` to examine the event scores and how the labels relate to them. The package includes four `get` functions that retrieve the various results of the labels. The four functions are `get_score`, `get_prob`, `get_labels`, and `get_tech`. The functions `get_score` and `get_prob` can return the vector of scores or probabilities for a specific event type ('bead', 'dead', 'debris', or 'doublet') or can return the entire `DataFrame` containing all of the scores or probabilities. The function `get_labels` returns a character vector containing all of the label assignments and `get_tech` returns the `DataFrame` containing all of the data used to label the events. 
...
```{r cytofHist}
bead <- cytofHist(get_score(x, type = 'bead'), get_labels(x), title = "Bead score")
debris <- cytofHist(get_score(x, type = 'debris'), get_labels(x), title = "Debris score")
doublet <- cytofHist(get_score(x, type = 'doublet'), get_labels(x), title = "Doublet score")
...
```{r cytofHist_density}
bead <- cytofHist(get_score(x, type = 'bead'), get_labels(x), type = "density", title = "Bead score")
debris <- cytofHist(get_score(x, type = 'debris'), get_labels(x), type = "density", title = "Debris score")
doublet <- cytofHist(get_score(x, type = 'doublet'), get_labels(x), type = "density", title = "Doublet score")
grid.arrange(bead, debris, doublet, ncol = 1)
```
