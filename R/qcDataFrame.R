...
#'
#' @param x A \code{matrix} created with \code{\link{dataPrep}}. The data must
#' be unstandardized so that the Gaussian parameters with zero values can
#' be identified.
#'
#' @return A \code{data.frame} that contains the following variables:
...
#'
#' @export
qcDataFrame <- function(x) {
  x <- as.data.frame(x)
  labs <- data.frame(Time = x$Time, label = "cell", bead = NA, doublet = NA,
                     debris = NA, dead = NA)

  for (i in 1:ncol(labs)) {
    labs$label[x$Center == 0 |
                 x$Offset == 0 |
                 x$Width == 0 |
                 x$Residual == 0] <- 'GDPzero'
  }
