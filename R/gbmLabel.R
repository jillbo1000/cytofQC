...
#' for the specified parameter type (bead, doublet, debris, or dead). The model
#' is computed using only the data specified in the index argument. The gbm is
#' tuned using \code{\link[EZtune]{eztune}} and then predicted values are
...
#' gbmLabel(tech, lab, type = "bead", init = beads$init, index = ind)
#' 
#' @export
gbmLabel <- function(x, labels, type, init, index, loss = "auc", standardize = TRUE) {
